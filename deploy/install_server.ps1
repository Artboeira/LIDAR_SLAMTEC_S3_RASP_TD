...
#
# ATENCAO MANUTENCAO: este arquivo deve ficar 100% ASCII (sem acentos),
# porque o Windows PowerShell 5.1 le .ps1 sem BOM como ANSI e caracteres
# UTF-8 quebram o parse (aspas "terminam" no meio de uma string).
#
# Faz em um comando o que docs/OPERACAO_EVENTO_WINDOWS.md descreve a mao:
#   1. confere Python 3.11+
#   2. cria .venv e instala server/requirements-server.txt
#   3. roda w2_validate.py (12/12 = servidor ok)
#   4. libera UDP 5555 e 7000 no firewall (se admin; senao imprime o comando)
#   5. gera start_fleet.bat e (opcional) coloca no auto-start do Windows
#   6. (opcional) gera chave SSH e instala nos 8 nos (senha pi123 1x por no)
#
# Uso (PowerShell, na raiz do repo - C:\lidarmapper):
#   powershell -ExecutionPolicy Bypass -File deploy\install_server.ps1
#
# Reexecutavel: pula o que ja esta feito.
...
foreach ($rule in @(@{n="LidarMapper V2 5555"; p=5555}, @{n="LidarMapper OSC 7000"; p=7000})) {
    if (Get-NetFirewallRule -DisplayName $rule.n -ErrorAction SilentlyContinue) {
        ...
        New-NetFirewallRule -DisplayName $rule.n -Direction Inbound -Protocol UDP -LocalPort $rule.p -Action Allow | Out-Null
...
$batBody = "cd /d $RepoRoot`r`n.venv\Scripts\python server\fleet_bridge.py --panels 1,2,3,4,5,6,7,8 --dest 127.0.0.1`r`n"
...
if (Test-Path "$key.pub") {
    ...
    if ($resp -match "^[sS]") {
        ...
        foreach ($n in 1..8) {
            ...
            $ok = Write-Output $pub | ssh -o StrictHostKeyChecking=accept-new -o ConnectTimeout=8 "pi@$hostn" "mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys && echo OK"
